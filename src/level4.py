...
def initial():
    ...
    global evil_twin, good_twin, on, off, good_world_flag, under_world_flag, on1, off1, on2, off2
    ...
    def on():
        ins = Player.entities["platform"].pop(
            (WIDTH - WIDTH // 3, HEIGHT // 2 + HEIGHT // 6)
        )
        ins.delete()
    ...
    def on1():
        ins = Player.entities["platform"].pop((WIDTH // 2, HEIGHT // 2 + HEIGHT // 6))
        ins.delete()
    ...
    Platform(
        WIDTH // 3, HEIGHT // 6, width=HEIGHT // 12, color=(255, 215, 0), alignment="v"
    )
    Platform(
        WIDTH // 2, HEIGHT // 6, width=HEIGHT // 12, color=(255, 215, 0), alignment="v"
    )
    Platform(
        WIDTH - WIDTH // 3,
        HEIGHT // 6,
    ...
    Button(WIDTH // 3 - 22, HEIGHT // 6 + 10, on, off)
    Button(WIDTH // 2 - 22, HEIGHT // 6 + 10, on1, off1)
    Button(WIDTH - WIDTH // 3 - 22, HEIGHT // 6 + 10, on2, off2)
